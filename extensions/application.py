...
class Application(commands.Cog):
  ...
  async def setup(self, ctx : commands.Context):
    ...
    for i in questions:
      await ctx.send(i)
      answer = await self.bot.wait_for("message", check=lambda m: str(m.author) == str(ctx.author) and m.channel == ctx.channel)
    ...
    if skill_dm.lower() == "yes":
      ...
      skill_dm = False
    try:
      await self.bot.db.execute("INSERT INTO application_setup(guild_id, channel_id, skill_dm) VALUES ($1, $2, $3)", guild_id, channel_id, skill_dm)
    except Exception as e:
      print(e)
      async with ctx.bot.embed(title="Error", description="Heyyy, there was an error. Please wait until it is fixed.") as emb:
  ...
  async def apply(self, ctx):
    ...
    if not setup:
      async with ctx.bot.embed(title="Error", description=f"Heyyy, there is no application setup here. Please notify the owner to do `{ctx.prefix}application setup`.") as emb:
        return await emb.send(ctx.channel)
    if not ctx.channel.id == setup["channel_id"]:
      async with ctx.bot.embed(title="Error", description=f"Heyyy, the channel you did the command in is not the application channel. Please execute it in the `{await self.bot.fetch_channel(setup['channel_id'])}`") as emb:
        return await emb.send(ctx.channel) 
    questions = [
    ...
    await self.bot.db.execute("INSERT INTO application(id, guild_id, channel_id, why_staff, why_choose_you, what_bring, how_help) VALUES ($1, $2, $3, $4, $5, $6, $7)", id, answer.guild.id, answer.channel.id, answers[0], answers[1], answers[3], answers[2])
    join = ", ".join((why_staff, why_choose_you, follow_rule, how_help))
    user = await self.bot.try_user(ctx.guild.owner_id)
    async with ctx.bot.embed(title="New Application", description=f"**Author**: {str(ctx.author)}\n\nQuestion: Why do you wanna join our staff team?\n    - {why_staff}\nQuestion: Why should we choose you?\n    - {why_choose_you}\nQuestion: How will you help us?\n    - {how_help}\nQuestion: Will you follow all of our rules?\n    - {follow_rule}\n\n    - ID {id}") as emb:
      await emb.send(user)
      await emb.send(ctx.channel)

  @application.command(name="accept")
  async def accept(self, ctx : commands.Context, id : str):
    ...
    await user.send(f"Hello! You have been accepted for staff on {ctx.guild.name}.")
