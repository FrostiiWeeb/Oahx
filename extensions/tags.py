...
class Tags(commands.Cog):
    ...
    async def too_long(self, ctx):
        async with self.bot.embed(
            title="Uhhh",
            description="You took too long! Try again with {ctx.prefix}{ctx.command.name}.",
        ) as emb:
        ...
        try:
            ...
            if not data["author"] == str(ctx.author):
                ...
                await self.bot.db.execute(
                    "UPDATE tags SET author = $1 WHERE user_id = $1",
                    str(ctx.author),
                    data["user_id"],
                )
        except Exception as e:
    ...
    async def edit(self, ctx, tag: TagName(lower=True), *, content: commands.clean_content):
        ...
        try:
            data = await self.bot.db.fetchrow(
                "UPDATE tags SET content = $1 WHERE name = $2 AND author = $3",
                content,
                str(tag),
                str(ctx.author),
            )
    ...
    async def transfer(self, ctx, target: discord.Member, *, tag: TagName(lower=True)):
        ...
        try:
            data = await self.bot.db.fetchrow(
                "UPDATE tags SET author = $1 WHERE name = $2 AND user_id = $3",
                str(target),
                str(tag),
                ctx.author.id,
            )
