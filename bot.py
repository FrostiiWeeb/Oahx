...
class Oahx(commands.Bot):
    def __init__(self, *args, **kwargs):
        ...
        self.initialised: asyncio.Event = asyncio.Event()
    ...
    async def on_message(self, message: discord.Message):
        if self.initialised.is_set() and message.content in self.mentions:
            # this is processed separately because the original, unaltered
            # message is needed elsewhere* - any direct changes are in-place
            # and carry over respectively, which we want to avoid.
            # conditional avoids duplicate processing
            #
            # *elsewhere - i.e., listeners
